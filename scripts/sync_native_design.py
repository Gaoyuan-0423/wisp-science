...
def exports():
    ...
    translations = (ROOT / "ui/src/i18n.rs").read_text()
    by_locale = {"Zh": {}, "En": {}}
    for locale, key, value in re.findall(r'\(Locale::(Zh|En), "([^"]+)"\) => Some\(("(?:[^"\\]|\\.)*")\)', translations):
        try:
            ...
            pass
    navigation_source = (ROOT / "ui/src/app_support/settings.rs").read_text()
    ...
    labels.update(json.loads((ROOT / "apps/macos/native-settings-english.json").read_text()))
    yield "native-english.json", (json.dumps(labels, ensure_ascii=False, indent=2, sort_keys=True) + "\n").encode()
    settings = (ROOT / "ui/src/settings_view.rs").read_text()
