...
impl AppState {
  ...
  pub fn handle_redraw(window_id: WindowId) {
    // Nested AppKit run loops (NSSavePanel sheets, `-[NSView display]`) can
    // invoke `drawRect:` while `cleared`/`wakeup` already hold `callback`.
    // Re-locking that non-recursive mutex deadlocks the main thread
    // (winit#1779, Wisp #1250). Keep the outer `in_callback` guard and enqueue
    // a deduplicated redraw for `cleared` to drain instead of dropping the frame.
