//! SSH host registry, validated connection snapshots, and compute-context prompt.
//!
...
pub fn password_delete(alias: &str) -> Result<(), String> {
...
pub fn decorate_host(mut host: SshHost) -> SshHost {
...
impl SshConnection {
    ...
    pub fn from_host(host: &SshHost) -> Result<Self, String> {
        let connection = Self {
...
pub fn build_password_askpass_env(password: &str) -> Result<Vec<(String, String)>, String> {
...
pub fn strip_legacy_compute_section(prompt: &mut String) {
...
pub async fn load(store: &wisp_store::Store) -> Vec<SshHost> {
    store
        .get_setting(KEY)
        .await
        .ok()
...
pub async fn save(store: &wisp_store::Store, hosts: &[SshHost]) -> Result<(), String> {
...
pub fn persistable_host(host: &SshHost) -> SshHost {
    SshHost {
...
pub fn apply_host_password(host: &SshHost) -> Result<(), String> {
...
fn preserve_interpreter_config(existing: &str, replacement: &str) -> Result<String, String> {
    let existing: serde_json::Value = if existing.trim().is_empty() {
        serde_json::json!({})
    } else {
        serde_json::from_str(existing).unwrap_or_else(|_| serde_json::json!({}))
    };
    let mut replacement: serde_json::Value =
        serde_json::from_str(replacement).map_err(|error| error.to_string())?;
    if let Some(target) = replacement.as_object_mut() {
...
async fn upsert_context_for_host(store: &wisp_store::Store, host: &SshHost) -> Result<(), String> {
    ...
    ctx.config_json =
        preserve_interpreter_config(&ctx.config_json, &ssh_context_config_json(host)?)?;
    ctx.updated_at = now;
    store
...
pub async fn save_and_sync_contexts(
...
pub async fn remove_context_for_alias(
    store: &wisp_store::Store,
    alias: &str,
) -> Result<(), String> {
...
pub fn list_ssh_config_aliases() -> Vec<String> {
