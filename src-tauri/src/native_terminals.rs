...
pub(crate) async fn dispatch(broker: &Broker, request: &Request, project_id: &str, session: &str) -> Result<Value, String> {
    let args: TerminalRequest = serde_json::from_value(request.args.clone()).map_err(|e| e.to_string())?;
    ...
    let manager = broker.app.state::<crate::terminal_sessions::TerminalManager>();
    let key = scope.scope_key();
    if request.command == "native_conversation_terminal_list" {
        return serde_json::to_value(manager.native_list(project_id, key)).map_err(|e| e.to_string());
    }
    if matches!(request.command.as_str(), "native_conversation_terminal_open" | "native_conversation_terminal_write" | "native_conversation_terminal_resize") {
    ...
    if request.command == "native_conversation_terminal_open" {
        ...
        return serde_json::to_value(crate::terminal_sessions::native_terminal_info(summary)).map_err(|e| e.to_string());
    }
    let id = args.terminal_id.ok_or("Terminal ID is required")?;
    match request.command.as_str() {
        "native_conversation_terminal_read" => serde_json::to_value(manager.native_read(&id, project_id, key, args.cursor)?).map_err(|e| e.to_string()),
        "native_conversation_terminal_write" => {
            let encoded = args.base64.ok_or("Terminal input is required")?;
            if encoded.len() > 128 * 1024 { return Err("Terminal input is too large".into()); }
        ...
        "native_conversation_terminal_resize" => {
            manager.native_resize(&id, project_id, key, args.rows.ok_or("Rows required")?, args.cols.ok_or("Columns required")?)?;
            Ok(Value::Null)
        }
        "native_conversation_terminal_close" => { manager.native_close(&id, project_id, key)?; Ok(Value::Null) }
