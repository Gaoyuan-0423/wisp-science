    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            if panelVisible, let session = model.activeSessionID {
                ...
                NativePanelView(client: conversation.client, projectID: project.id, sessionID: session, readOnly: conversation.snapshot?.read_only ?? true, manageWorkflows: model.openWorkflowSettings) { panelVisible = false }
