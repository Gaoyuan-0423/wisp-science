...
internal static class TranscriptView
{
        ...
        foreach (var section in TranscriptPresentation.Sections(message))
        {
            if (section.ToolName is { } tool)
            {
                ...
                header.Children.Add(new TextBlock { Text = (section.IsResult ? "工具结果 · " : "工具调用 · ") + tool,
                    FontSize = 12, Foreground = design.Brush("text-muted") });
            ...
            {
                var rich = new RichTextBlock { FontSize = 14, Foreground = design.Brush("text"),
    ...
    private static FrameworkElement Code(string value, WispDesign design) => new ScrollViewer
    {
        ...
        Content = new TextBlock { Text = value, FontFamily = new FontFamily("Cascadia Mono, Consolas"), FontSize = 12,
    ...
    private static void AppendBlock(RichTextBlock rich, MarkdownBlock block, WispDesign design, string prefix = "")
    {
        ...
        {
            var paragraph = new Paragraph { Margin = new Thickness(0, 6, 0, 12), FontFamily = new FontFamily("Cascadia Mono, Consolas"), FontSize = 12 };
        ...
        else if (block is LeafBlock leaf)
        {
            ...
            if (block is HeadingBlock heading) { paragraph.FontSize = heading.Level <= 2 ? 18 : 15; paragraph.FontWeight = FontWeights.SemiBold; }
    ...
    private static XamlInline RenderInline(Markdig.Syntax.Inlines.Inline inline, WispDesign design)
    {
        switch (inline)
        {
            ...
            case CodeInline code: return new Run { Text = code.Content, FontFamily = new FontFamily("Cascadia Mono, Consolas"), Foreground = design.Brush("clay-strong") };
