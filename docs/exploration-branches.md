...
2. Choose **Start exploration** on any completed assistant response, name it, and create it. Ordinary conversation branches do not show this action; start the exploration from main.
...
Every completed response exposes **Start exploration**. While a round is unresolved, only its selected response can create more candidates; the banner’s **Start another** uses that same response even if later replies exist. The source’s full current conversation remains frozen and is retained on promotion or abandonment. Promotion appends the candidate’s new turns after the existing mainline tail, without copying the inherited prefix or replacing later mainline turns. After context compaction, a historical reply resolves through its visual User event and `MessageBoundary` seq and clones that epoch’s prefix — the same anchors rewind and branch use. The latest turn still clones the current working set. Creation reports that the history is unavailable only when the selected turn has no completed boundary, is out of range, or has no visual anchor. ACP conversations cannot be explored, and ACP-bound conversations cannot bypass a project's exploration freeze.

## Ordinary conversation branches

Ordinary conversation branches use a branch icon in the sidebar and also appear directly below the message checkpoint where they were created. Nested sidebar branches sit in a **Branches** group under their source conversation, and the same heading sits above the checkpoint cards in the transcript; click either heading to collapse or expand the list. New branches inherit their source conversation's current folder and nest beneath that conversation in the same sidebar folder; branches of ungrouped conversations remain ungrouped. Moving the main conversation to another folder or to Ungrouped also moves all descendant branches, including those not yet loaded in the sidebar. You can still move a branch and its descendants separately without moving its parent or siblings; moving the main conversation again reunites all descendants in the destination folder. The main conversation remains free to continue while every branch develops its own later context.
...
Rewind follows that real append position, not the card's visual location. After context compaction, rewind and branch resolve the chosen bubble through the visual User event and its `MessageBoundary` seq, then restore that epoch's prefix — they do not count rows in the compacted working set. Rewinding main past a merge revokes it and reopens the branch. Rewinding past the branch checkpoint keeps the branch as frozen history, removes its checkpoint attachment, and prevents it from merging into the rewritten mainline.
...
Individually discarding a candidate removes its private records and validated app-data workspace, but the round remains unresolved and the source mainline stays frozen. Select a candidate or right-click the source mainline and choose **Abandon exploration** to resolve the round. Both paths clean all exploration candidates; selection merges the chosen candidate's deltas into the original mainline, while abandonment preserves the original mainline conversation and project state.
