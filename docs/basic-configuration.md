...
**网络与镜像：** 进入 **设置 → 网络**，分别保存模型 API、MCP 服务和本地 curl/Python 等代码请求的代理。原模型页的代理会自动保留。每项可选跟随系统、直连或自定义地址；已有 MCP 连接和 Python/R 解释器需重新连接或重启后使用新值。**包镜像 → 配置** 可填写 Conda、pip 源及可选 CA 证书包路径，这些值从下一轮对话起注入 Agent 的环境安装指导，不修改系统软件源或域名访问策略。SSH/WSL 使用各自环境的代理配置。详见[网络设置](network-settings.md)。
...
首次引导按「欢迎使用 → wisp-science 能做什么 → 配置模型 → 本地环境」分为四页；模型密钥可稍后配置，本地环境单独展示。弹窗不显示滚动条，小窗口中仍可滚动内容，底部导航保持可见。

打开引导后可直接按 **Enter** 进入下一页，在模型密钥输入框中按 Enter 也会保存并继续；空密钥则跳过配置。最后一页按 Enter 开始使用。输入法确认和长按回车不会翻页，路径编辑等控件保留各自的键盘操作。切页只更新弹窗内容，背景遮罩不再重复淡入；**Esc** 返回上一页，在欢迎页按 Esc 关闭引导。

本地环境页会后台检测 Python、Rscript、uv、Node、npm、sci、pixi 的路径，保存到 Local，并保留已有配置。可点 **编辑路径** 手动填写可执行文件的完整路径，再点 **保存路径**；支持包含空格的 Windows/macOS/Linux 路径。保存前检查文件是否存在，不执行程序。清空字段后保存会恢复自动检测；**重新检测路径** 保留已保存的手动选择。**设置 → 常规 → 本地环境** 也支持同样的编辑操作；该面板不再位于模型页。网络代理和包镜像同样位于 **设置 → 常规 → 网络**，不再作为独立标签。缺少工具不影响启动，也不会自动安装；需要时可在对话中让 Wisp 使用 `local-env-setup` 配置。以后可随时按 **Cmd/Ctrl+P**，搜索 **快速配置** 重新打开这个页面。重新打开只刷新检测结果，不会重置模型或自动安装依赖。
...
为避免模型在无迭代上限时空转，Wisp 还会检查最近完成的工具调用与结果是否形成连续重复循环。只有调用和可见结果一起重复时才计为“没有进展”；同一读取调用返回了新结果，或其间存在不同的页面导航、运行时操作等状态变化时，会继续执行。若确认重复循环，Wisp 会在保留完整且配对的工具调用/结果后中断本轮。

模型错误会显示在对话中的错误卡片内。长错误和网址自动换行；“继续执行”和“复制”保持横排，小窗口中按钮整组移到下一行。

执行步骤标题旁的状态图标由内点和外圈组成：内点轻缓呼吸，外圈短弧环绕，表示任务仍在进行。模型等待没有可测量的百分比，因此外圈不代表完成比例。工具返回后，主标题显示“等待模型继续”，趣味提示放在较淡的第二行；步骤结束后动画停止，成功处理的步骤显示勾选，含失败或未完成工具的步骤显示中性横线。系统开启“减少动态效果”时，图标保持静止，状态文字仍然可见。

常见问题：

- `401/403`：检查 API Key、账户权限和服务商区域。
- `404`：通常是 API 基础地址或模型 ID 错误；不要盲目追加接口路径。
- 上下文溢出：在会话里执行 `/compact`，选择常规压缩或语义压缩；或在 **设置 → 对话** 开启自动压缩长会话。切换模型不会自动语义压缩，除非打开「切换模型时自动语义压缩」。
  压缩过程会在对话底部显示轻量状态和收拢动画；完成后在对话中保留压缩前后 token 数量，以及数量下降时的减少比例。标题栏不再重复显示压缩提示。开启系统“减少动态效果”时，状态仍可见，动画停用。
...
当前对话的输入框上方会列出本机、已附加的远程服务器，以及该对话的默认分析环境（即使尚未点选加入本会话），并显示各自的 Python / R 运行时状态。点击语言芯片即可打开该运行时的内存环境（变量表）；点击服务器名称打开完整的交互运行时面板。内存环境里可以用 Python / R 标签切换语言，并用筛选框按名称或类型查找对象。

### 默认分析环境

默认分析环境分成两层：

1. **全局默认**（**设置 → 环境** 页顶部的下拉框）：只作为**新对话**的起点。选某台 SSH/WSL 主机，或选「默认使用本地」。改这里不会改写已经创建的对话。
2. **对话默认**（输入框左下角 Agent 菜单 → **计算环境**）：每个对话可以钉一台机器，或钉在本机。行右侧星标和下拉框只影响当前对话。

Agent 调用 `python`、`r` 等接受 `context_id` 的工具时，省略 `context_id` 按 `该对话已保存的默认 > 全局默认 > 本地` 解析。新建对话会把当时的全局默认快照下来，并在目标是远程时加入本会话，因此可以立刻显式传 `context_id`。Agent 仍可通过显式传 `local` 使用本机。

该子菜单里另外一件事是**当前会话**：

1. **点击服务器行**：把该主机加入**当前会话**。行内状态为「本会话」或「未加入」。未加入时 Agent 不能显式指定该环境。
2. 若某台机器是该对话解析后的默认但尚未点选加入（例如旧对话跟随全局默认），行内状态为「将自动加入」。Agent 菜单「计算环境」一行会显示「默认 CPU3」这类名称，而不是「默认使用本地」。
3. 取消附加当前这条对话默认时，该对话回到跟随全局，而不会改全局设置。若要把本对话钉在本机，用下拉框选「默认使用本地」或点掉星标。
...
3. 打开右上角 **开发者模式**。
4. 点击 **加载未打包的扩展程序**，选择 `browser_setup` 返回的稳定受管 `browser-extension/` 文件夹本身，不要选择里面的单个文件或 ZIP。Wisp 会先校验随应用发布的版本、扩展 ID、必需文件和 SHA-256 清单，再同步到该目录。
5. 打开扩展弹窗，确认显示 **Connected to Wisp**。

若已连接扩展低于当前 Wisp 内置版本，应用会显示独立的更新横幅，列出当前版本、所需版本和准确路径。点击 **更新扩展** 后，Wisp 会刷新并复核受管目录；支持自动重载的新版扩展会自行重连。`0.2.x` 等旧版受 Chrome 安全机制限制，需要在已打开的扩展管理页点一次 **重新加载**，必要时从横幅给出的目录重新“加载已解压的扩展程序”。长路径会在横幅内以省略号显示，窄窗口中的操作按钮会自动换行；**复制扩展路径** 始终复制完整路径。横幅同时提供 **复制扩展路径**、**打开扩展管理页** 和 **重新检测**，并会在新版本握手成功后自动消失。
...
在右侧「文件」面板的当前项目中，可将文件直接拖到对话输入区。列表、网格和搜索结果均支持；移入输入区时边框高亮，松开后显示可移除的文件路径引用。重复拖入同一路径只保留一条引用，发送前仍可继续编辑问题。

此操作仅引用项目内原路径，不上传、不复制到 `uploads/`，图片也作为路径上下文交给 Agent，避免自动转成视觉附件。该拖拽入口适用于当前项目的文件，不包含远程文件面板或文件夹。
...
- `/btw [问题]`：打开侧边问答；带问题时直接把问题发到侧边问答。侧边问答先让模型语义判断问题意图（会话进展/前后对比/具体内容），再按意图从当前会话冻结证据里作答；与会话无关的具体内容问题会提示证据不足。
  - 意图分类超过 30 秒、模型服务报错或返回格式无效时，自动改用最近会话记录和原问题关键词匹配的证据继续尝试回答，不影响主分析任务。降级检索可能遗漏较早记录；证据不足时会明确说明。若回答阶段的模型服务仍不可用，则显示实际回答错误，需检查服务或稍后重试。
  - 侧边输入框与主会话使用一致的整体圆角边框和焦点样式，模型选择与发送按钮位于框内；输入框不再显示原生拖拽角，支持按内容增高（最高 180px，超过后在框内滚动）。模型菜单也可选择 ACP Agent 独立回答。
...
- `/files`：打开右侧文件面板。
- `/upload`：打开文件上传对话框。
- `/share`：打开「分享为长图」：勾选消息、打码关键词，选择导出 PNG 长图或 HTML 网页。连续勾选长列表时会保留当前位置；自定义 HTML 文件名时会自动保留 `.html` 类型。导出 PNG 时可设置图片宽度。导出按当前对话的 Markdown 渲染结果出图（含表格与公式），不会另做一套气泡皮肤。顶栏铃铛左侧的分享按钮、输入框左侧 `+` 菜单的「分享」是同一个入口。会话还没有可分享的消息时，顶栏按钮会禁用。

助手回复中的项目文件路径（包括已收集为产物的路径标签）可以左键预览；右键可使用系统默认应用打开、在中间区域打开、复制绝对或相对路径，或在系统文件管理器中定位该文件。
...
2. 使用搜索框按标题、项目路径、会话 ID 或源文件路径筛选会话；点击一条会话查看有界预览，确认标题、工作目录、消息数和时间。
3. 点击单条 **导入**，或使用 **全部导入**。
...
进入 **设置 → 连接**，点击连接器名称打开详情。内置连接器会显示中英文简介、维护方和数据来源／官方文档链接；顶部开关可启用或停用连接器。

点击工具名前的展开按钮可查看完整说明、参数类型、必填项和默认值。还可以展开完整参数定义；服务端提供返回数据定义或参数示例时也会展示。内置工具说明与实际调用使用同一份 Rust catalog，自定义 MCP 则显示服务器返回的工具信息。

工具右侧的允许、询问、禁止按钮与展开操作独立，切换审批状态不会收起说明。内置连接器的“跳过审批”仍按现有规则生效。按 Escape 可从详情返回连接列表。

### 设置工具审批模式

进入 **设置 → 权限**，顶部的 **工具审批模式** 只影响当前项目，从下一次工具调用起生效，无需新建会话。连接页保留连接启停和逐工具规则。

- **按工具规则**（默认）：遵循各工具的允许、询问或禁止规则；危险命令仍需确认，并非每次调用都会询问。
- **自动批准**：跳过普通工具的询问，危险命令仍需确认。
- **完全放行**：展开 **高级设置** 后选择，同时跳过危险命令确认。折叠高级设置后，当前模式和说明仍会显示；可直接点击“按工具规则”或“自动批准”退出完全放行。

模式切换沿用已有审批策略，不会清空逐工具规则或记住的授权，也不会取消工具策略中的有效禁止规则。权限页下方仍可查看和撤销记住的授权。这项项目设置与对话中的“完全权限”开关分别保存。

### 配置本地 MCP

进入 **设置 → 连接 → 添加连接**，将类型设为 **本地命令**，填写名称、可执行命令和参数，再点击 **测试**。测试期间按钮会禁用并显示进度，避免重复点击；本地命令需要启动进程，可能要等几秒到几十秒。

![添加本地 MCP 连接](assets/basic-configuration/10-add-mcp-connection.png)

命令应指向可执行文件，不要把整段 shell 管道塞进字段。连接级环境变量默认隐藏填写，可用眼睛图标切换可见；值只保存在操作系统钥匙串。编辑时留空会保留已保存的值，删掉一行才会清除。共用的 API Key 仍可放在 **设置 → 凭据**，新启动的本地 MCP 进程也会继承这些值。
...
OpenAlex、InfiniSynapse、SCIMaster、NCBI 分别作为列表入口，显示用途摘要与已配置字段数量。点击服务进入独立页面，查看详细说明、官方获取入口并填写凭据，然后点击页面底部 **保存**。已经保存的字段显示“已配置”；留空会保持原值，不会把密钥明文回显。通过顶部返回按钮、凭据面包屑、**取消** 或 Escape 返回列表；未保存的该服务输入会被丢弃。自定义凭据仍在列表下方配置。
